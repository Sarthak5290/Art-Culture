...
from components.cards import render_item_card, render_featured_card, render_compact_item_list
...
def render(app_data):
    ...
    if category_info:
        # Category header with hero styling
        st.markdown(f"""
        <div style="background: linear-gradient(135deg, #E8F4FD, #F8F9FA); padding: 2.5rem 2rem; margin: -1rem -1rem 2rem -1rem; border-radius: 0 0 25px 25px; text-align: center;">
            <h1 class="section-title" style="margin: 0; font-size: 2.8rem;">{category_info['displayTitle']}</h1>
            <p style="font-size: 1.1rem; color: #666; margin-top: 1rem; font-style: italic;">
        ...
        if not items:
            ...
            <div style="text-align: center; padding: 3rem; background: white; border-radius: 20px; margin: 2rem 0;">
                ...
                <p style="color: #888;">We're working on adding more content to this category. Check back soon!</p>
            </div>
            """, unsafe_allow_html=True)
        else:
            # Show item count and category stats
            st.markdown(f"""
            <div style="background: white; padding: 1.5rem; border-radius: 15px; margin-bottom: 2rem; box-shadow: 0 4px 16px rgba(0,0,0,0.05);">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <h4 style="margin: 0; color: #1E3A5F;">📚 Collection Overview</h4>
                        <p style="margin: 0.5rem 0 0 0; color: #666;">Discover {len(items)} amazing pieces in this collection</p>
                    </div>
                    <div style="text-align: right;">
                        <span style="background: #D4AF37; color: white; padding: 0.5rem 1rem; border-radius: 20px; font-weight: 600;">
                            {len(items)} Items
                        </span>
                    </div>
                </div>
            ...
            if len(items) > 0:
                ...
                with col2:
                    render_featured_card(items[0], selected_category_id, 0, is_featured=True)
    ...
    for i in range(0, len(items_to_show), items_per_row):
        ...
        for j in range(items_per_row):
            ...
            if item_index < len(items_to_show):
                with cols[j]:
                    render_item_card(items_to_show[item_index], category_id, item_index + 1)
...
def render_detailed_list_item(item, category_id, item_index):
    ...
    with col2:
        ...
        <div style="padding-left: 1.5rem;">
            <h4 style="color: #1E3A5F; margin-bottom: 0.8rem; font-size: 1.4rem;">{item_title}</h4>
            <p style="color: #666; line-height: 1.6; margin-bottom: 1.5rem;">{preview_text}</p>
        </div>
        """, unsafe_allow_html=True)
